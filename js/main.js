...
function search() {
  ...
  storage.classList.add('favourite');
  console.log(storage);
  ...
  fetch(url)
    ...
    .then(function(json) {
      for (var i = 0; i < json.length; i++) {
        ...
        //divContainerCard.style.backgroundImage = url('https://via.placeholder.com/210x295/cccccc/666666/?text=TV');
        cardContainer = creatediv(divContainerCard, divContainerTitle);
        if (storage === 'nofavourite favourite') {
        cardContainer.classList.add('nofavourite');
      } else {
        cardContainer.classList.add('favourite');
      }
...
function addFavorite(e) {
  ...
  var classSeries = currentFilm.className;
  // var arrayStorage = [];
  // arrayStorage.push(currentFilm.lastChild.textContent);
  //return arrayStorage;
    // console.dir(arrayStorage);
    //localStorage.setItem('series', JSON.stringify(arrayStorage));
    localStorage.setItem('series', classSeries);
  }
